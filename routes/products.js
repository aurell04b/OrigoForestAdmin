...
router.post('/actions/duplicate', permissionMiddlewareCreator.smartAction(), (req, res, next) => {
  ...
  while (req.body.data.attributes.ids[i]) {
    let productId = req.body.data.attributes.ids[i];
    ...
    models.products
      ...
      .then((result) => {
        models.products.create({
          ...
          indexedPrice: result.indexedPrice,
          tva: result.tva,
          gridConnected: result.gridConnected,
          centraleDate: result.centraleDate,
          deliveryRequirement: result.deliveryRequirement,
        });
      });
    i += 1;
  }
...
router.post('/actions/mensualiser', permissionMiddlewareCreator.smartAction(), (req, res) => {
  let productId = req.body.data.attributes.ids[0];
  ...
  return models.products
    ...
    .then((result) => {
      ...
      let _tva = result.tva;
      let _gridConnected = result.gridConnected;
      let _centraleDate = result.centraleDate;
      let _deliveryRequirement = result.deliveryRequirement;

      let i = 0;
      while (tmp_date < _productionEnd) {
        let tmp_end = new Date(tmp_date);
        let tmp_start = new Date(_productionStart);
        ...
        models.products.create({
          ...
          tva: _tva,
          gridConnected: _gridConnected,
          centraleDate: _centraleDate,
          deliveryRequirement: _deliveryRequirement,
        });
